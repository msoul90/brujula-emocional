 ...
 *
 * Renders one of: 'agitated' | 'sad' | 'confused' | 'good'
 *
 * Props:
 *   mood:    'agitated' | 'sad' | 'confused' | 'good'  (required)
 *   size?:   number — px width/height (default 64)
 *   color?:  string — stroke color (default uses each mood's deep tone)
 *
 * Usage:
...
export default function MoodIcon({ mood, size = 64, color }) {
  if (!PATHS[mood]) {
    console.warn(`MoodIcon: unknown mood "${mood}". Expected one of agitated, sad, confused, good.`);
    return null;
  }
