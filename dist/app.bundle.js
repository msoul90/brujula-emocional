(() => {
  ...
  var ZONE_SHAPES = {
    simple: {
      ...
      arms: [
        { tag: "rect", x: 12, y: 41, width: 14, height: 88, rx: 6 },
    ...
    detailed: {
      ...
      stomach: [{ tag: "rect", x: 28, y: 112, width: 44, height: 42, rx: 4 }],
      arms: [
        { tag: "rect", x: 12, y: 43, width: 14, height: 86, rx: 6 },
  ...
  function createBodyMap({ emociones: emociones2, getDisplayName, t, showDetail, onDismiss, onSwitchToQuiz }) {
    ...
    function render() {
      ...
      const selectedChips = zones.filter((z) => selectedZones.has(z.id)).map((z) => `
      ...
      for (const btn of content.querySelectorAll(".body-result-card")) {
        btn.addEventListener("click", () => {
          const emotion = emociones2.find((e) => e.nombre === btn.dataset.bodyEmotion);
          if (emotion) {
            onDismiss();
            showDetail(emotion);
          }
