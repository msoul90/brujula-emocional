...
// ── Legend and quadrant button builders ──────────────────────────────────────
...
function svgBody(nodes, edges, W, H, sel, view, { t, activeTypes, activeQuadrant, nameFilter }) {
    ...
    // Pre-compute once — used by both edge and node rendering
    const normalizedFilter = nameFilter ? normalizeText(nameFilter) : "";

    // Edges — only visible types; opacity varies by sel/quadrant/name filter
    ...
    function bindEvents(wrap) {
        ...
        if (searchInput) {
            ...
            const populateSuggestions = (value) => {
                ...
                const matches = emociones
                    .filter(e => normalizeText(getDisplayName(e.nombre)).includes(norm))
                    .slice(0, 8);
                if (!matches.length) { hideSuggestions(); return; }
                suggestionsList.innerHTML = matches.map(e =>
                    `<li role="option" tabindex="-1" data-nombre="${escapeHtmlAttr(e.nombre)}"
            ...
            const trySelectExact = () => {
                ...
                if (found) { selected = found.nombre; hideSuggestions(); render(); }
            };
